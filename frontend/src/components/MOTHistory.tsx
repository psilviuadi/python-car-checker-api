...
const MOTHistory: React.FC<MOTHistoryProps> = ({ tests }) => {
  const [expandedTests, setExpandedTests] = useState<Set<string>>(
    tests.length > 0 ? new Set([tests[0].motTestNumber]) : new Set()
  )

  const toggleExpanded = (motTestNumber: string) => {
    const newExpanded = new Set(expandedTests)
    if (newExpanded.has(motTestNumber)) {
      newExpanded.delete(motTestNumber)
    } else {
      newExpanded.add(motTestNumber)
    }
    ...
    <div className="mot-history-card">
      ...
      <div className="mot-tests">
        {tests.map((test, index) => {
          const isExpanded = expandedTests.has(test.motTestNumber)
          return (
            <div key={test.motTestNumber} className={`mot-test mot-${test.testResult.toLowerCase()} ${isExpanded ? 'expanded' : 'collapsed'}`}>
              <button
                className="mot-test-header"
                onClick={() => toggleExpanded(test.motTestNumber)}
              >
